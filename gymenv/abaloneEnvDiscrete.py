...
class AbaloneDiscrete(gym.Env):
    def __init__(self, env, agent_player, gamma=0.99, raise_error=True, one_hot_encode=True, enable_swapping=False):
        super().__init__()
        self.env = env
    ...
    def step(self, action, render_opp_move=False, print_info=False):
        # if self.raise_error:
        #     valid_actions, _ = h.turn_actions_to_indices(mg.generate_valid_moves(self.env.turn, self.env.state), self.env.grid_size)
    ...
    def reset(self, seed=None, options=None, render_opp_move=False, print_info=False):
